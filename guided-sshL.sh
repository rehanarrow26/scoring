...
SERVER_AUTH="/home/$SERVER_USER/.ssh/authorized_keys"
...
-o StrictHostKeyChecking=no \
$SERVER_USER@$SERVER_IP \
"grep -Fxq '$PUB' ~/.ssh/authorized_keys"
...
then
    ...
    echo "[FAIL]"
    echo "Public Key belum ditemukan."
fi
...
PERM=$(ssh $SERVER_USER@$SERVER_IP stat -c %a ~/.ssh)
...
PERM=$(ssh $SERVER_USER@$SERVER_IP stat -c %a ~/.ssh/authorized_keys)
...
OWNER=$(ssh $SERVER_USER@$SERVER_IP stat -c %U ~/.ssh/authorized_keys)
